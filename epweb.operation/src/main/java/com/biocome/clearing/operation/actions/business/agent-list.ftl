...
<div class="pageHeader">
    <@s.form id="pagerForm" class="pageForm required-validate" action="/business/agent-list" onsubmit="if(/^[0-9]*$/.test($('#agentmobile').val())) return  navTabSearch(this);else {alertMsg.info('手机格式错误'); return false;}">
       ...
       <div class="searchBar">
            <ul class="searchContent">
                ...
                <li>
                    <label>手机：</label>
                    <input type="text" value="${searchModel.phone}"  maxlength="20" name="phone" id="agentmobile" class="phone"/>
                </li>
                 <li>
                    <label>产品：</label>
                    <select name="product" >
...
<div class="panelBar">
            <ul class="toolBar">
                <li>
                    <a href="<@s.url "/business/agent-add" />" target="dialog" rel="business-agent-add" title="新增代理商" width="600" height="300"><span class="a09">新增</span> </a>
                </li>
            </ul>
</div>
<div class="pageContent">
        ...
        <table class="table" width="100%" layoutH="165">
            <thead style="border-top:1px solid #ccc;">
                <tr>
                    <th align="center" width="12%">代理商名称</th>
                    <th align="center" width="8%">代理商手机</th>
                    <th align="center" width="8%">代理商类型</th>
            ...
            <tbody>
                ...
                <tr target="sid_cl" rel="1">
                    <td>${cl.name}</td>
                    <td>${cl.phone}</td>
                    <td><@system.getValue path="${cl.type}" items=types itemKey="code" itemValue="codeName"/></td>
                    <td><@system.getValue path="${cl.product}" items=products itemKey="code" itemValue="codeName"/></td>
                    <td>${cl.num}</td>
                    <td>${cl.operateDate!''}</td>
                    <td>
                    <a href="<@s.url "/business/agent-edit?id=${cl.id}" />" target="dialog"  rel="business_agent_edit" title="编辑代理商" width="600" height="300">编辑</a>
                    &nbsp;|&nbsp;
                    <a href="<@s.url "/business/agent-del?id=${cl.id}" />" target="ajaxTodo" title="确定要删除代理商${cl.name}？">删除</a></td>
                </tr>
                </#list>
            </tbody>
        </table>
